# Second Brain — Knowledge Graph RAG with Semantic Search & Auto-Import

A **semantic knowledge management system** that transforms Claude conversations into a searchable knowledge graph. Features intelligent caching to optimize API costs, automatic conversation imports, and a web interface for interactive queries.

## What It Does

- 🧠 **Build Knowledge Graphs** — Parse markdown notes and create a directed graph of interconnected concepts
- 🔍 **Semantic Search** — Find relevant information using embeddings (text-similarity-3-small)
- 💬 **Chat with Context** — Ask questions and get Claude-generated answers enriched with relevant notes
- ⚡ **Query Caching** — Cache identical and similar questions to reduce API tokens (~30-50% cost reduction on repeat queries)
- 📥 **Auto-Import Conversations** — Import Claude conversations automatically from conversations.json or via file watcher
- 🌐 **Web Interface** — Streamlit app with graph visualization, chat, note creation, and web content import
- 📚 **Privacy-First** — All data stored locally; vault is git-ignored

## Key Features

| Feature | Benefit |
|---------|---------|
| **Exact & Semantic Matching** | Answer repeated questions instantly (0 tokens) |
| **Lazy Embeddings** | Generate embeddings only on first query, not during import |
| **Real-Time Watcher** | Automatically detect and import new conversations |
| **Session Hook** | Auto-import after Claude Code sessions |
| **RSS Integration** | Import web articles and summarize with AI |
| **Wikilink Support** | Create knowledge connections with `[[Note Name]]` syntax |
| **Interactive Graph** | Visualize notes and their relationships with PyVis |
